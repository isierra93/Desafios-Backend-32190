...
class Contenedor {
    ...
    //Reemplaza por ID
    async replaceById(id, prod) {
        if (this.checkIfExist()) {
            try {
                const elemts = await this.getAll();
                elemts[id - 1] = prod;
                await this.deleteAll();
                await this.saveArray(elemts);
            } catch (err) {
                throw new Error(`Error borrando el archivo: ${err}`);
            };
        };
    };
    //Cart remove by id & idprod
    async removeByIds(id, id_prod,carrito) {
        if (this.checkIfExist()) {
            try {
                carrito.map( cart => {
                    if(cart.id == id){

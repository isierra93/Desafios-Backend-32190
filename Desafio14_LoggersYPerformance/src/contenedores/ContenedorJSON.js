...
class ContenedorJSON {
  ...
  constructor(nameFile, extension = `json`) {
    (this.nameFile = nameFile), (this.extension = extension);
  }
  ...
  async save(obj) {
    ...
    if (this.checkIfExist()) {
      try {
        let arr = await fs.promises.readFile(
          `./${this.nameFile}.${this.extension}`,
          `utf-8`
        );
        ...
        await fs.promises.writeFile(
          `./${this.nameFile}.${this.extension}`,
          JSON.stringify(arrParse, null, 2)
        );
        //Retorna el ultimo producto asignado CON ID
        return arrParse[id - 1];
      } catch (err) {
        logger.logConsola.error(`Error actualizando archivo: ${err}`);
        logger.logError.error(`Error actualizando archivo: ${err}`);
      }
    }
    ...
    try {
      ...
      logger.logConsola.error(`Error creando nuevo archivo: ${err}`);
      logger.logError.error(`Error creando nuevo archivo: ${err}`);
    }
  }
  ...
  async getById(numberId) {
    if (this.checkIfExist()) {
      try {
        ...
        logger.logConsola.error(`Error buscando ID en el archivo: ${err}`);
        logger.logError.error(`Error buscando ID en el archivo: ${err}`);
      }
    }
  }
  async getAll() {
    if (this.checkIfExist()) {
      try {
        ...
        logger.logConsola.error(`Error de lectura en el archivo: ${err}`);
        logger.logError.error(`Error de lectura en el archivo: ${err}`);
      }
    }
    return null;
  }
  ...
  async deleteAll() {
    if (this.checkIfExist()) {
      try {
        ...
        logger.logConsola.error(`Error borrando el archivo: ${err}`);
        logger.logError.error(`Error borrando el archivo: ${err}`);
      }
    }
  }
}
...
export { productosContainer, mensajesContainer };

...
if (DOT_ENV.MODE === "CLUSTER" && cluster.isPrimary) {
  ...
  app.get(
    "/addCarritoProd/:email/:prodId",
    checkAuthentication,
    Routes.getAddCarritoProd
  );
  app.get(
    "/deleteCarritoProd/:email/:prodId",
    checkAuthentication,
    Routes.getDeleteCarritoProd
  );
  // perfil
  ...
  app.post(
    "/avatarChange",
    checkAuthentication,
    Multer.upload.single("newAvatar"),
    Routes.FileCheck,
    Routes.postAvatarChange
  );
  // pedidoCarrito
  app.get(
    "/pedidoCarrito/:email/:productosId",
    checkAuthentication,
    Routes.getPedidoCarrito
  );
  // Server ON
  httpServer.listen(PORT, () => {
    Logger.logConsola.info(
      `Server iniciado desde el PORT ${PORT} en modo ${DOT_ENV.MODE}`
    );
  });

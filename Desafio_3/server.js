//Se importa express y se declara app y puerto
...
//Manejador de error de inicio del servidor
server.on(`error`, error => console.log(`Error en servidor: ${error}`));

//Rutas. Las mismas son asinc para poder aguardar la lectura del archivo TXT
app.get(`/productos`, async (req, res) => {
    const prods = await tienda.getAll();
    res.send(prods);
});

app.get(`/productoRandom`, async (req, res) =>{
    const x = Math.floor(Math.random()*(4-1)+1)
    const prod = await tienda.getById(x)
    res.send(prod)
});
...
//Se instancia un nuevo contenedor
const tienda = new Contenedor(`productos`);